...
class BaseDataset(data.Dataset):
    ...
    def get_transform(self, input_nc):
        transform_list = []
        ...
        if 'resize' in self.opt.preprocess_mode:
            ...
            transform_list.append(transforms.Lambda(lambda img: self.__scale_width(img, self.opt.load_size, Image.BICUBIC)))
        elif 'scale_shortside' in self.opt.preprocess_mode:
            transform_list.append(transforms.Lambda(lambda img: self.__scale_shortside(img, self.opt.load_size, Image.BICUBIC)))
        
        if 'crop' in self.opt.preprocess_mode:
            transform_list.append(transforms.Lambda(lambda img: self.__crop(img, self.params['crop_pos'], self.opt.crop_size)))
        
        if self.opt.preprocess_mode == 'none':
            base = 32
            transform_list.append(transforms.Lambda(lambda img: self.__make_power_2(img, base, Image.BICUBIC)))
        
        if self.opt.preprocess_mode == 'fixed':
            w = self.opt.crop_size
            h = round(self.opt.crop_size / self.opt.aspect_ratio)
            transform_list.append(transforms.Lambda(lambda img: self.__resize(img, w, h, Image.BICUBIC)))
        
        if self.opt.isTrain and not self.opt.no_flip:
            transform_list.append(transforms.Lambda(lambda img: self.__flip(img, self.params['flip'])))
        
        if input_nc == 4:  # RGBD input
            transform_list.append(transforms.ToTensor())
            transform_list.append(transforms.Normalize((0.5, 0.5, 0.5, 0.5), (0.5, 0.5, 0.5, 0.5)))
        elif input_nc == 1:  # Lidar input (assuming grayscale)
            transform_list.append(transforms.ToTensor())
